...
//	vsprintf (msg, fmt, argptr);
	...
	if (logfile_active && logfile_active->value)
	{
		...
		if (!logfile)
		{
			Com_sprintf (name, sizeof(name), "%s/kmkpconsole.log", FS_Gamedir ()); //hypov8 add: km
...
#if KINGPIN
	if (to->modelindex != 255 || /*protocol_version*/ 121 < 114) //hypov8 todo: not needed?
	{
...
#if KINGPIN
	if (bits & U_NUMPARTS)
		MSG_WriteByte (msg, to->num_parts);
	if (bits & U_MODELPARTS)
	{
		...
		for (a=0; a<MAX_MODEL_PARTS; a++)
		{
			if (c&(1<<a))
			{
				...
				if (c2 & 1)
					MSG_WriteByte (msg, to->model_parts[a].modelindex);
				if (c2 & 2)
					MSG_WriteByte (msg, to->model_parts[a].invisible_objects);
				if (c2 & 4)
				{
					int a2, c3 = 0;
					...
					for (a2=0; a2<MAX_MODEL_PARTS; a2++)
						if (c3 & (1 << a2))
							MSG_WriteByte (msg, to->model_parts[a].skinnum[a2]);
				}
			}
		}
	}
	if (bits & U_PART3MODEL)
		MSG_WriteByte (msg, to->model_parts[3].modelindex);
#endif
...
#if KINGPIN
	if ((bits & (U_RENDERFX8|U_RENDERFX16)) == (U_RENDERFX8|U_RENDERFX16))
		MSG_WriteLong (msg, to->renderfx2);
	else if (bits & U_RENDERFX8)
		MSG_WriteByte (msg, to->renderfx2);
	else if (bits & U_RENDERFX16)
		MSG_WriteShort (msg, to->renderfx2);

	if (bits & U_ORIGINDELTA)
	{
		if (bits & U_ORIGIN1)
			MSG_WriteByte (msg, (int)((to->origin[0] - from->origin[0]) * 4 + 128));
		if (bits & U_ORIGIN2)
			MSG_WriteByte (msg, (int)((to->origin[1] - from->origin[1]) * 4 + 128));
		if (bits & U_ORIGIN3)
			MSG_WriteByte (msg, (int)((to->origin[2] - from->origin[2]) * 4 + 128));
	}
	else
#endif
	{
...
#if KINGPIN
	if (bits & U_SCALE)
		MSG_WriteByte (msg, (int)((to->scale + 1) * 127));
...
#ifdef LARGE_MAP_SIZE
//hypov8 short[3]
void MSG_ReadPos (sizebuf_t *msg_read, vec3_t pos)//hypov8 short[3]
{
	if (LegacyProtocol())
	{
...
void Qcommon_Init (int argc, char **argv)
{
	...
	// Knightmare- for the game DLL to tell what engine it's running under
	sv_engine = Cvar_Get ("sv_engine", KM_GAME_NAME, CVAR_SERVERINFO | CVAR_NOSET | CVAR_LATCH);
	...
	Com_Printf ("====== "KM_GAME_NAME" Initialized ======\n\n");
